...
export default {
  ...
  async beforeMount () {
    this.posts = (await articlesCollection().where('published_at', '<', firebase.firestore.FieldValue.serverTimestamp()).orderBy('published_at', 'desc').get()).docs.map(v => ({id: v.id, ...v.data()}))
